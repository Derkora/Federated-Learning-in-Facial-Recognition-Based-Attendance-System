...
class AttendanceController:
    ...
    def recognize_and_submit(self, img_pil, model, reference_embeddings, threshold=0.65):
        """Perform face recognition and submit attendance if match found."""
        ...
        if face is not None:
            ...
            if max_sim > threshold:
                print(f"[Inference] Match Found: {best_match} ({max_sim:.2f})")
    ...
    def submit_attendance(self, user_id, confidence):
        try:
            payload = {"user_id": user_id, "edge_id": self.client_id, "confidence": confidence, "lecture_id": "L123"}
            requests.post(f"{self.server_url}/submit-attendance", json=payload, timeout=5)
            print(f"[Attendance] Submitted for {user_id}")
        except: pass
