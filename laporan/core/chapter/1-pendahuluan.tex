...
Berdasarkan permasalahan tersebut, penelitian tugas akhir ini mengusulkan penerapan \textit{Federated Learning} pada sistem kehadiran berbasis pengenalan wajah di lingkungan \textit{Edge Computing}. Tahap pengumpulan data dilakukan secara terpusat menggunakan \textit{laptop}, yaitu dengan merekam video wajah mahasiswa yang kemudian dipotong menjadi kumpulan citra (\textit{frame}) dan didistribusikan secara manual ke masing-masing perangkat \textit{edge} sebelum pelatihan dimulai. Penerapan \textit{Federated Learning} dalam penelitian ini dimulai dari tahap \textit{preprocessing} citra lokal di setiap perangkat \textit{client} hingga proses inferensi kehadiran, sementara model \textit{MobileFaceNet} dilatih secara lokal di setiap perangkat tanpa citra wajah yang dikirim ke \textit{server} pusat. Proses distribusi dan orkestrasi \textit{server} dan \textit{client} akan menggunakan \textit{Docker} agar sistem lebih mudah dijalankan dan direplikasi. Harapannya, kombinasi \textit{Federated Learning}, \textit{MobileFaceNet}, dan \textit{Edge Computing} dapat menghasilkan sistem kehadiran yang lebih aman, efisien, serta tetap menjaga privasi pengguna.
...
\begin{enumerate}[itemsep=1pt]
    ...
    \item Proses akuisisi data dilakukan menggunakan \textit{laptop} dengan cara merekam video wajah mahasiswa menggunakan \textit{webcam}, yang kemudian diekstraksi menjadi kumpulan citra (\textit{frame}) dan didistribusikan secara manual ke setiap perangkat \textit{edge}.
    \item Penerapan \textit{Federated Learning} pada penelitian ini mencakup tahap \textit{preprocessing} citra di perangkat \textit{edge} hingga proses inferensi kehadiran; tahap pengumpulan data awal tidak termasuk dalam siklus \textit{Federated Learning}.
\end{enumerate}
