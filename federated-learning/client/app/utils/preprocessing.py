...
# Gunakan CPU untuk pemrosesan citra dasar karena sistem kita murni CPU
DEVICE = torch.device('cpu')

from app.utils.logging import get_logger

# Koordinat landmark wajah standar (canonical) untuk proses Affine Alignment (ukuran 96x112)
CANONICAL_LANDMARKS = np.array([
    [30.2946, 51.6963],  # koordinat mata kiri
    [65.5318, 51.5014],  # koordinat mata kanan
    [48.0252, 71.7366],  # koordinat hidung
    [33.5493, 92.3655],  # koordinat mulut kiri
    [62.7299, 92.2041],  # koordinat mulut kanan
], dtype=np.float32)
...
class ImageProcessor:
    def __init__(self):
        ...
        # Transformasi normalisasi piksel gambar sesuai standar input MobileFaceNet
        self.normalize = T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.50196, 0.50196, 0.50196])

    @property
    def mtcnn(self):
        # Memuat detektor wajah MTCNN secara Lazy-Loading ke RAM hanya jika dibutuhkan
    ...
    def unload_detector(self):
        # Membersihkan model MTCNN dari RAM setelah preprocessing selesai untuk menghemat memori
    ...
    def detect_face(self, img, save_path=None, keep_all=False):
        try:
            ...
            # Hanya mengambil satu wajah utama terbaik (Registrasi & Absensi Standar)
            if not keep_all:
                face_img = None
                
                # Lakukan Affine Alignment berbasis koordinat 5 titik landmark
                ...
                # Fallback Bounding Box Crop jika proses alignment gagal
                if face_img is None:
                    box = boxes[0]
                    margin = 20
                ...
                # Simpan gambar wajah hasil pemotongan jika save_path ditentukan
                if save_path and face_img:
                    face_img.save(save_path)
                    
                return face_img, boxes[0], probs[0]
            
            # Deteksi semua wajah aktif di dalam frame (Multi-Face Tracking / Video Stream)
            else:
                results = []
                for i, box in enumerate(boxes):
                    prob = probs[i]
                    if prob < 0.85:  # Filter threshold keyakinan MTCNN
                        continue
                        
                    face_img = None
                    # Affine Alignment terpisah untuk masing-masing subjek terdeteksi
                    ...
                    # Fallback crop jika alignment gagal untuk subjek bersangkutan
                    if face_img is None:
                        margin = 20
                        x1, y1 = max(0, int(box[0] - margin/2)), max(0, int(box[1] - margin/2))
                        x2, y2 = min(img.width, int(box[2] + margin/2)), min(img.height, int(box[3] + margin/2))
    ...
    # Normalisasi Tensor Model
    def prepare_for_model(self, face_data):
        if face_data is None: return None

        # Konversi objek Tensor/Gambar ke format PIL Image
        ...
        # Lakukan resize akhir ke ukuran wajib MobileFaceNet (96x112)
        if face_img.size != (96, 112):
            face_img = face_img.resize((96, 112), Image.BILINEAR)

        # Transformasi gambar ke Tensor dan normalisasi nilai L2
        face_tensor = TF.to_tensor(face_img)
        normalized_tensor = self.normalize(face_tensor)
        
        # Tambahkan dimensi batch (1, C, H, W) sebelum dikirimkan ke CPU model
        return normalized_tensor.unsqueeze(0).to(DEVICE)

    # Evaluasi Ketajaman Citra
    ...
    # Top-N Laplacian Image Quality Selection
...
# Inisialisasi instance terpusat pemroses citra
