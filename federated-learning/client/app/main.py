...
from fastapi import FastAPI, Request, Depends, HTTPException, BackgroundTasks, UploadFile, File, Body, Form
    ...
    def gen_frames():
        while True:
            if fl_manager.latest_frame is not None:
                ...
                ret, buffer = cv2.imencode('.jpg', frame_small, [cv2.IMWRITE_JPEG_QUALITY, 85])
                yield (b'--frame\r\n'
...
async def request_preprocess_legacy(background_tasks: BackgroundTasks, dataset: str = "students"):
    background_tasks.add_task(fl_manager.run_preprocess_phase, dataset)
    return {"status": "success"}
...
async def proxy_get_video_cache(video_name: str):
    try:
        cache_key = f"{fl_manager.client_id}_{fl_manager.model_version}_{video_name}"
...
async def client_list_models():
    # Mengambil daftar model dari server FL dan menyajikannya ke frontend
    try:
        res = requests.get(f"{fl_manager.server_api_url}/api/models/list", timeout=5)
        return JSONResponse(res.json(), status_code=res.status_code)
    except Exception as e:
        return JSONResponse({"status": "error", "message": f"Server offline atau tidak terjangkau: {e}"}, status_code=500)
...
async def client_select_model(data: dict = Body(...)):
    ...
    success, msg = fl_manager.select_and_sync_version(version)
    if success:
...
async def client_list_datasets():
    # Mengambil daftar dataset di raw_data beserta status pra-proses
    return {
        "status": "success",
        "datasets": fl_manager.get_available_datasets(),
        "current_dataset": fl_manager.get_dataset_name()
    }

@app.post("/api/dataset/select")
async def client_select_dataset(data: dict = Body(...)):
    # Memilih rute dataset aktif di client secara dinamis
    dataset_name = data.get("dataset")
    if not dataset_name:
        return JSONResponse({"status": "error", "message": "Nama dataset tidak boleh kosong."}, status_code=400)
    
    avail = fl_manager.get_available_datasets()
    if dataset_name not in avail:
        return JSONResponse({"status": "error", "message": f"Dataset '{dataset_name}' tidak ditemukan di client."}, status_code=400)
    
    fl_manager.current_dataset = dataset_name
    processed_path = fl_manager.get_processed_path(dataset_name)
    if hasattr(fl_manager.client, 'trainer'):
        fl_manager.client.trainer.data_path = processed_path
        
    fl_manager.report_status(f"Dataset aktif diubah secara dinamis ke: {dataset_name}")
...
def get_safe_dataset_path(dataset_name: str) -> str:
    ...
    return os.path.join(fl_manager.raw_data_path, safe_name)
...
def get_safe_student_path(dataset_name: str, student_dir: str) -> str:
    ...
    return os.path.join(fl_manager.raw_data_path, safe_dataset, safe_student)
...
async def stream_dataset_simulation(student: str = None):
    dataset_name = fl_manager.get_dataset_name()
    dataset_path = get_safe_dataset_path(dataset_name)
    ...
    try:
        ...
        fl_manager.logger.error(f"Error loading identities: {e}")
    finally:
        db.close()
